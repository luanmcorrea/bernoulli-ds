import { CaretDownIcon,  XIcon, MagnifyingGlassIcon } from "@phosphor-icons/react"
...
function MultiselectTrigger({ className, size = "default", children, ...props }: MultiselectPrimitive.Trigger.Props & {
  ...
  return (
    <MultiselectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "group/multiselect-trigger flex w-fit items-center gap-1.5 rounded-3xl border bg-input px-3 py-2 text-sm whitespace-nowrap transition-[color,box-shadow,background] outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:ring-ring focus-visible:ring-2   disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-muted aria-invalid:border-destructive aria-invalid:ring-4 aria-invalid:ring-destructive/20 data-placeholder:text-muted-foreground data-[size=default]:h-10 data-[size=sm]:h-8 data-[size=lg]:h-12 data-[size=lg]:text-xl data-[size=lg]:px-4 dark:aria-invalid:ring-destructive/40 [&>svg:not([class*='size-'])]:size-4 data-[size=lg]:[&>svg:not([class*='size-'])]:size-5",
        className,
      )}
      {...props}
    >
