...
function AvatarBadge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        "absolute -right-px -bottom-px z-10 inline-flex items-center justify-center rounded-full bg-sidebar-primary text-primary-foreground bg-blend-color ring-2 ring-card select-none",
...
function AvatarGroupCount({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex size-8 shrink-0 items-center justify-center rounded-full bg-muted text-sm font-bold text-accent-foreground ring-2 ring-card group-has-data-[size=lg]/avatar-group:size-10 group-has-data-[size=sm]/avatar-group:size-6 [&>svg]:size-4 group-has-data-[size=lg]/avatar-group:[&>svg]:size-5 group-has-data-[size=sm]/avatar-group:[&>svg]:size-3",
        className
      )}
      {...props}
    />
